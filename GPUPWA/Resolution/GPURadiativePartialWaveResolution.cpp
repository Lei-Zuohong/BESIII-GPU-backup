#include "GPURadiativePartialWaveResolution.h"
...
GPURadiativePartialWaveResolution::GPURadiativePartialWaveResolution(GPUStreamTensor2 & _orbital, GPUPropagatorResolution & _prop1, char * _name):
	GPUBasicPartialWaveResolution(_name, _orbital.GetNSets(), _prop1),
	mNProp(1), mProp1(_prop1), mProp2(_prop1), mOrbital(_orbital)
{
}

GPURadiativePartialWaveResolution::GPURadiativePartialWaveResolution(GPUStreamTensor2 & _orbital, GPUPropagatorResolution & _prop1, GPUPropagatorResolution & _prop2, char * _name):
...
GPURadiativePartialWaveResolution::~GPURadiativePartialWaveResolution(void)
{
}
        
GPUDataStream<float2> * GPURadiativePartialWaveResolution::Contract(GPUPartialWave * wave2, unsigned int index, unsigned int block)
{
	...
	GPURadiativePartialWaveResolution * castwave2 = dynamic_cast<GPURadiativePartialWaveResolution *>(wave2);
	if (castwave2) {
		...
		if (nprop1 == 1 && nprop2 == 1) {
			...
			if (prop1a.IdenticalMx2(prop2a)) {
				...
				kernelradcontract_reswave_10(mOrbital.GetList()->GetDeviceInterface(),
						mOrbital(index, block), contract, 
						castwave2->GetTensor()(index, block),
						(*mg_perp_stream)(index, block),
						aiaj);
				delete contract;
			}
			else {
				...
				kernelradcontract_reswave_11(mOrbital.GetList()->GetDeviceInterface(),
			...
			if (prop1a.IdenticalMx2(prop2a)) {
				...
				kernelradcontract_reswave_12(mOrbital.GetList()->GetDeviceInterface(),
			...
			if (prop1a.IdenticalMx2(prop2a)) {
				...
				kernelradcontract_reswave_20(mOrbital.GetList()->GetDeviceInterface(),
				...
				GPUDataStream<float2> * sum2a = prop2a.SumResolution(index, block);
				kernelradcontract_reswave_21(mOrbital.GetList()->GetDeviceInterface(),
			...
			if (prop1a.IdenticalMx2(prop2a) && prop1b.IdenticalMx2(prop2b)) {
				...
				GPUDataStream<float2> * sum2a = prop2a.SumResolution(index, block);
				GPUDataStream<float2> * sum2b = prop2b.SumResolution(index, block);
				kernelradcontract_reswave_22(mOrbital.GetList()->GetDeviceInterface(),
						mOrbital(index, block), sum1a, sum1b, 
						castwave2->GetTensor()(index, block), sum2a, sum2b, 
						(*mg_perp_stream)(index, block),
						aiaj);
