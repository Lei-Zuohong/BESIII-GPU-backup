/* Propagators.h  -- source for interface functions to the kernels
 defined in /home/leizh/gpu/GPUPWA/Propagators.cl, the corresponding binaries are in binfiles/Propagators.bin
...
namespace Propagators_GPU{
	 ...
	 int kernelsigma(const DeviceInterface * dev, Stream<float> * mx2in, float mr, float gf, Stream<float2> * output){
		 ...
		 err = k_kernelsigma->setArg(3, *(*output)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
