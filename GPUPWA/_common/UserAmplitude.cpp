/* UserAmplitude.h  -- source for interface functions to the kernels
 defined in /besfs/groups/gpugroup/mintj/tmp/gpupwa/GPUPWA/UserAmplitude.cl, the corresponding binaries are in binfiles/UserAmplitude.bin
...
namespace UserAmplitude_GPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/UserAmplitude.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/UserAmplitude.bin failed \n";
			 	return FAILURE;
		 }
