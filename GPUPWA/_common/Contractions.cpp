/* Contractions.h  -- source for interface functions to the kernels
 defined in /home/leizh/gpu/GPUPWA/Contractions.cl, the corresponding binaries are in binfiles/Contractions.bin
...
namespace Contractions_GPU{
	 ...
	 cl::Kernel * k_kernelcontractmesons_v_vs = 0;
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 ...
		 k_kernelcontractmesons_v_vs = new cl::Kernel(*bProgram, "kernelcontractmesons_v_vs", &err);
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel::Kernel() failed (" << err << ") for kernel kernelcontractmesons_v_vs\n";
			 return FAILURE;
		 }
	 ...
	 int kernelcontractmesons_v_vs(const DeviceInterface * dev, Stream<float4> * propagator1real, Stream<float4> * propagator1imag, Stream<float4> * propagator2real, Stream<float4> * propagator2imag, Stream<float2> * contract, Stream<float2> * result){
		 ...
		 evvec->push_back((propagator1real->GetEvent()));
		 err = k_kernelcontractmesons_v_vs->setArg(0, *(*propagator1real)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		...
		if(propagator1imag != propagator1real)
			 evvec->push_back((propagator1imag->GetEvent()));
		 err = k_kernelcontractmesons_v_vs->setArg(1, *(*propagator1imag)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		...
		if(propagator2real != propagator1real)
			if(propagator2real != propagator1imag)
				 evvec->push_back((propagator2real->GetEvent()));
		 err = k_kernelcontractmesons_v_vs->setArg(2, *(*propagator2real)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		...
		if(propagator2imag != propagator1real)
			if(propagator2imag != propagator1imag)
				if(propagator2imag != propagator2real)
					 evvec->push_back((propagator2imag->GetEvent()));
		 err = k_kernelcontractmesons_v_vs->setArg(3, *(*propagator2imag)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		 ...
		 evvec->push_back((contract->GetEvent()));
		 err = k_kernelcontractmesons_v_vs->setArg(4, *(*contract)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		 ...
		 err = k_kernelcontractmesons_v_vs->setArg(5, *(*result)());
		 if (err != CL_SUCCESS) {
			 std::cerr << "Kernel.SetArg() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
		 ...
		 err = dev->GetQueue()->enqueueNDRangeKernel(*k_kernelcontractmesons_v_vs, cl::NullRange, cl::NDRange(propagator1real->GetD1(),propagator1real->GetD2()), cl::NullRange, evvec, &event );
		 if (err != CL_SUCCESS) {
			 std::cerr << "CommandQueue::enqueueNDRangeKernel() failed (" << err << ")\n";
			 assert(0);
			 return FAILURE;
