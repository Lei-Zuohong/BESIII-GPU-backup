/* Resolution.h  -- source for interface functions to the kernels
 defined in /besfs/groups/gpugroup/mintj/tmp/gpupwa/GPUPWA/Resolution.cl, the corresponding binaries are in binfiles/Resolution.bin
...
namespace Resolution_GPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Resolution.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Resolution.bin failed \n";
			 	return FAILURE;
		 }
