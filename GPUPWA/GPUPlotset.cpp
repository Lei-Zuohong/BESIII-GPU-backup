...
{
	for (VVTH1Fs::iterator it = mHistos.begin(); it < mHistos.end(); ++it)
	{
	...
	for (VVTH2Fs::iterator ite = m2DHistos.begin(); ite < m2DHistos.end(); ++ite)
	{
	...
	for (VTGraphs::iterator it = mGraphs.begin(); it < mGraphs.end(); ++it)
	{
...
void GPUPlotset::WriteRootfile(char *filename)
{
	...
	for (VVTH1Fs::iterator it = mHistos.begin(); it < mHistos.end(); ++it)
	{
		for (VTH1Fs::iterator it2 = it->begin(); it2 < it->end(); it2++)
		{
			(*it2)->Write();
		}
	}
	for (VVTH2Fs::iterator it = m2DHistos.begin(); it < m2DHistos.end(); ++it)
	{
		for (VTH2Fs::iterator it2 = it->begin(); it2 < it->end(); it2++)
		{
			(*it2)->Write();
		}
	}
