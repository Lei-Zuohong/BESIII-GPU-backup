/// \file GPUFactorizedPartialWaveRes.h
...
**/
template <typename T>
class GPUFactorizedPartialWaveRes: public GPUPartialWave
{
public:
  /// Constructor
  GPUFactorizedPartialWaveRes(GPUStreamTensor<T> & _orbitalTensor, ///< (Real) orbital tensor, has to have the correct rank for the analysis
		 GPUPropagatorResolution & _propagator,      ///< Complex propagator
		 char * _name);                    ///< Name for the wave

  /// Destructor
  virtual ~GPUFactorizedPartialWaveRes(void);

  /// Get the Tensor part
  GPUStreamTensor<T> & GetTensor(){return mOrbitalTensor;};
  /// Get the propagator part
  GPUPropagatorResolution & GetPropagator(){return mPropagator;};
...
//  virtual bool HasContractOrbital(GPUPartialWave * wave);
 ...
 protected:
  ...
  GPUPropagatorResolution & mPropagator;
  /// Name of the wave
  char * mName;
...
typedef GPUFactorizedPartialWaveRes<float> GPUFactorizedScalarPartialWaveRes;
/// A partial wave with a vector orbital part
typedef GPUFactorizedPartialWaveRes<float4>  GPUFactorizedVectorPartialWaveRes;
/// A partial wave with a tensor2 orbital part
typedef GPUFactorizedPartialWaveRes<float44>  GPUFactorizedTensor2PartialWaveRes;
