...
GPUPWAAmplitudeCalculator::GPUPWAAmplitudeCalculator(GPUPartialWaveAnalysis *_ana,
													 GPULookupTable *const _table_data,
													 GPULookupTable *const _table_mc,
													 std::vector<GPUDataStream<float4> **> _Weights,
													 char *_mcintegralfilename) : GPUPWACalculator(_ana, _Weights),
																				  mAna(_ana),
																				  mLookupTable_data(_table_data),
																				  mLookupTable_mc(_table_mc),
																				  mNCoefficient_data(_table_data->GetNBlocks()),
																				  mNCoefficient_mc(_table_mc->GetNBlocks()),
																				  mNAddition_data(_table_data->GetNAdditional()),
																				  mNAddition_mc(_table_mc->GetNAdditional()),
																				  mMCIntegralCoefficients(0)
{
	if (1 == 0)
	{
		cout << "Info from GPUPWAAmplitudeCalculator: Initializing self." << endl;
	}
...
GPUPWAAmplitudeCalculator::~GPUPWAAmplitudeCalculator(void)
{
	...
	if (mFitinit)
	{
		for (int i = 0; i < mNCoefficient_data; i++)
		{
			delete mdcs[i];
		}
		delete[] mdcs;
...
double GPUPWAAmplitudeCalculator::LikelihoodGradientHessianCalc(const vector<double> &pars, vector<double> &gradient, vector<double> &hessian, bool dogradient, bool dohessian)
{
	...
	int nwaves = mLookupTable_data->GetNWaves();
	...
	//int sumblocks = mNCoefficient_data*2;
	//if(mNAddition_data != 0)
	//  sumblocks = (mNCoefficient_data-1)*2;
	...
	if (!mFitinit)
	{
		...
		mdcs = new GPUDataStream<float4> *[mNCoefficient_data];

		unsigned int maxblocksize = mAna->Blocksize;
		if (mNCoefficient_data == 1 && mNAddition_data != 0)
		{
			maxblocksize = mNAddition_data;
		}
		...
		if (mNAddition_data != 0)
		{
			mshortlogdcs = new GPUDataStream<float4>(mdev, mNAddition_data);
			unsigned int dims8[] = {mNAddition_data, nhessian};
		...
		for (int i = 0; i < mNCoefficient_data; i++)
		{
			...
			if (i == mNCoefficient_data - 1 && mNAddition_data != 0)
				size = mNAddition_data;
			mdcs[i] = new GPUDataStream<float4>(mdev, size);
		}
		mdtcs = new GPUDataStream<float>(mdev, mpsize);

		minnergradients = new GPUDataStream<float4> *[mNCoefficient_data];

		for (int j = 0; j < mNCoefficient_data; j++)
		{
			unsigned int size = maxblocksize;
			if (j == mNCoefficient_data - 1 && mNAddition_data != 0)
				size = mNAddition_data;
		...
		if (mNAddition_data != 0)
		{
			unsigned int dims9[] = {mNAddition_data, mngrad};
	...
	double sum = 0.0;
	unsigned int num;
	for (int b = 0; b < mNCoefficient_data; b++)
	{
		if (b == mNCoefficient_data - 1 && mNAddition_data != 0)
		{
			logdcs = mshortlogdcs;
			num = mNAddition_data;
		}
		...
		pwamult4(cl::NDRange(num),
				 ...
				 mLookupTable_data->GetCoefficientsX(b),
				 mLookupTable_data->GetCoefficientsY(b),
				 mypars,
				 nwaves,
	...
	if (dohessian)
	{
		...
		int length = 0;
		//for(int k=0; k < mNCoefficient_data; k++){
		for (int k = mNCoefficient_data - 1; k >= 0; k--)
		{
			// Switch between short and long blocks
			if (k == mNCoefficient_data - 1 && mNAddition_data != 0)
			{
		...
		if (mAna->GetWaves()->Active(i))
		{
			...
			for (int j = i + 1; j < nwaves; j++)
			{
				if (mAna->GetWaves()->Active(j))
				{
					...
					sum += 2.0 * (pprod.x * mMCIntegralCoefficients[i][j].x + pprod.y * mMCIntegralCoefficients[i][j].x + par_i.x * par_j.y * mMCIntegralCoefficients[i][j].y - par_i.y * par_j.x * mMCIntegralCoefficients[i][j].y);
				}
			}
		}
	}
...
{
	initMCIntegralCoefficients(mLookupTable_mc->GetNWaves());
	unsigned int blocks = mLookupTable_mc->GetNBlocks();
	unsigned int waves = mLookupTable_mc->GetNWaves();
	...
	{
		GPUDataStream<float4> *cx = mLookupTable_mc->GetCoefficientsX(b);
		GPUDataStream<float4> *cy = mLookupTable_mc->GetCoefficientsY(b);
		double *mysumx;
		double *mysumy;
		if (weights)
		{
...
STATUS GPUPWAAmplitudeCalculator::WriteMCIntegralFile(bool all, char *filename)
{
	...
	f << "End of file, should contain " << endl
	  << lines + 2 << " lines, including this one." << endl;
...
float **GPUPWAAmplitudeCalculator::GetDcs(unsigned int index, bool dooffdiagonal, int nblocks)
{
	...
	{
		if (!mLookupTable_mc->CacheValid())
			mLookupTable_mc->FillTable();
	}
	else
	{
	...
	if (nblocks > 0 && nblocks < mNCoefficient_data)
	...
	//assert(ncoeffblocks <=  mNCoefficient_data);
	if (nblocks == -1 && index == mAna->GetDataIndex())
		NCoefficientBlocks = mNCoefficient_data;
	else if (nblocks == -1 && index == mAna->GetMCIndex())
		NCoefficientBlocks = mNCoefficient_mc;
	else
		NCoefficientBlocks = ncoeffblocks;

	int NAdditionalEvents = mNAddition_data;
	if (index == mAna->GetMCIndex())
		NAdditionalEvents = mNAddition_mc;
	...
	if (index == mAna->GetDataIndex())
	{
		for (int i = 0; i < NCoefficientBlocks; i++)
		{
			...
			if (i == mNCoefficient_data - 1 && NAdditionalEvents != 0)
			...
			pwamultdcsonly4(cl::NDRange(size), mdev, mLookupTable_data->GetCoefficientsX(i), mLookupTable_data->GetCoefficientsY(i), mypars, nwaves, (float)mAna->GetParameters()->Value((unsigned int)0), dcs);
			...
			for (int j = 0; j < nwaves; j++)
			{
				...
				if (i == mNCoefficient_data - 1 && NAdditionalEvents != 0)
					size = NAdditionalEvents;
				GPUDataStream<float4> *dcs = new GPUDataStream<float4>(mdev, size);
				pwamultdcsonlyonewave4(cl::NDRange(size), mdev, mLookupTable_data->GetCoefficientsX(i), mypars, nwaves, j, dcs);
				...
				for (int j = 0; j < nwaves; j++)
				{
					for (int k = j + 1; k < nwaves; k++)
					{
						...
						if (i == mNCoefficient_data - 1 && NAdditionalEvents != 0)
							size = NAdditionalEvents;
						GPUDataStream<float4> *dcs = new GPUDataStream<float4>(mdev, size);
						pwamultdcsonlytwowaves4(cl::NDRange(size), mdev, mLookupTable_data->GetCoefficientsX(i), mLookupTable_data->GetCoefficientsY(i), mypars, nwaves, j, k, dcs);
	...
	else
	{
		...
		{
			int size = mAna->Blocksize;
			if (i == mNCoefficient_mc - 1 && NAdditionalEvents != 0)
				size = NAdditionalEvents;
			//sum += size*4;
			//cout << "In Get DCS: " << i << " : " << size << " total: " << sum << endl;
			GPUDataStream<float4> *dcs = new GPUDataStream<float4>(mdev, size);
			//cout << (float)mAna->GetParameters()->Value((unsigned int)0) << endl;
			pwamultdcsonly4(cl::NDRange(size), mdev, mLookupTable_mc->GetCoefficientsX(i), mLookupTable_mc->GetCoefficientsY(i), mypars, nwaves, (float)mAna->GetParameters()->Value((unsigned int)0), dcs);
			dcs->Read((float4 *)&result[0][i * mAna->Blocksize * 4]);
			delete dcs;
			...
			for (int j = 0; j < nwaves; j++)
			{
				...
				if (i == mNCoefficient_mc - 1 && NAdditionalEvents != 0)
					size = NAdditionalEvents;
				GPUDataStream<float4> *dcs = new GPUDataStream<float4>(mdev, size);
				pwamultdcsonlyonewave4(cl::NDRange(size), mdev, mLookupTable_mc->GetCoefficientsX(i), mypars, nwaves, j, dcs);
				dcs->Read((float4 *)&result[j + 2][i * mAna->Blocksize * 4]);
				delete dcs;
			}
				...
				for (int j = 0; j < nwaves; j++)
				{
					for (int k = j + 1; k < nwaves; k++)
					{
						...
						if (i == mNCoefficient_mc - 1 && NAdditionalEvents != 0)
							size = NAdditionalEvents;
						GPUDataStream<float4> *dcs = new GPUDataStream<float4>(mdev, size);
						pwamultdcsonlytwowaves4(cl::NDRange(size), mdev, mLookupTable_mc->GetCoefficientsX(i), mLookupTable_mc->GetCoefficientsY(i), mypars, nwaves, j, k, dcs);
						dcs->Read((float4 *)&result[ind][i * mAna->Blocksize * 4]);
						delete dcs;
...
// 我的函数
void GPUPWAAmplitudeCalculator::GetAmplitudes_Data(const vector<double> &pars, char *filename)
{
	mAna->SetParameterVector(pars);
	double tcs = 1.0;
	int nwaves = mLookupTable_data->GetNWaves();
	...
	for (int b = 0; b < mNCoefficient_data; b++)
	{
		...
		if (b == mNCoefficient_data - 1 && mNAddition_data != 0)
			nev = mNAddition_data;
		cout << "Output Amplitude Entries: " << nev << endl;
		for (int i = 0; i < nev; i++)
		{
			fprintf(f, "%e\n", outbuffer[i].x);
			fprintf(f, "%e\n", outbuffer[i].y);
			...
			for (int j = i + 1; j < nwaves; j++)
			{
				if (mAna->GetWaves()->Active(j))
				{
					double2 par_j = mdouble2(Parameters->Value(waves[j]->GetMagnitudeParameter()) * cos(Parameters->Value(waves[j]->GetPhaseParameter())),
											 Parameters->Value(waves[j]->GetMagnitudeParameter()) * sin(Parameters->Value(waves[j]->GetPhaseParameter())));
					double2 pprod = par_i * par_j;
					tmp = 2.0 * (pprod.x * mMCIntegralCoefficients[i][j].x + pprod.y * mMCIntegralCoefficients[i][j].x + par_i.x * par_j.y * mMCIntegralCoefficients[i][j].y - par_i.y * par_j.x * mMCIntegralCoefficients[i][j].y) / ngen;
					sum += tmp;
					ptx[activeindex][activeindex2] = tmp;
					activeindex2++;
				}
