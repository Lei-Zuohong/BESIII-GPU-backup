...
vector<TH1F*> GPUStreamScalar::Plot(char * id, char * title,int nbins, double min, double max, float ** dcs, int nwaves, GPUStreamScalar & cutvar, GPUStreamScalar & cut1var, GPUStreamScalar & cut2var, GPUStreamScalar & cut3var, GPUStreamScalar & cut4var,GPUStreamScalar & cut5var, GPUStreamScalar & cut6var,bool dooffdiagonal, float * weights,	int dataindex, int mcindex,  int nblocks){
	...
	cutvar.ResetUsecount(dataindex);
	cutvar.ResetUsecount(mcindex);
	cutvar.IncreaseUsecount(dataindex);
	cutvar.IncreaseUsecount(mcindex);


	cut1var.ResetUsecount(dataindex);
	cut1var.ResetUsecount(mcindex);
	cut1var.IncreaseUsecount(dataindex);
	cut1var.IncreaseUsecount(mcindex);



	cut2var.ResetUsecount(dataindex);
	cut2var.ResetUsecount(mcindex);
	cut2var.IncreaseUsecount(dataindex);
	cut2var.IncreaseUsecount(mcindex);



	cut3var.ResetUsecount(dataindex);
	cut3var.ResetUsecount(mcindex);
	cut3var.IncreaseUsecount(dataindex);
	cut3var.IncreaseUsecount(mcindex);



	cut4var.ResetUsecount(dataindex);
	cut4var.ResetUsecount(mcindex);
	cut4var.IncreaseUsecount(dataindex);
	cut4var.IncreaseUsecount(mcindex);



	cut5var.ResetUsecount(dataindex);
	cut5var.ResetUsecount(mcindex);
	cut5var.IncreaseUsecount(dataindex);
	cut5var.IncreaseUsecount(mcindex);



	cut6var.ResetUsecount(dataindex);
	cut6var.ResetUsecount(mcindex);
	cut6var.IncreaseUsecount(dataindex);
	cut6var.IncreaseUsecount(mcindex);
	...
	for(int block =0; block < GetNBlocks(dataindex); block++){
		...
		float * data = ReadBlock(dataindex, block);
                float * datacut = cutvar.ReadBlock(dataindex, block);
                float * datacut1 = cut1var.ReadBlock(dataindex, block);
                float * datacut2 = cut2var.ReadBlock(dataindex, block);
                float * datacut3 = cut3var.ReadBlock(dataindex, block);
                float * datacut4 = cut4var.ReadBlock(dataindex, block);
                float * datacut5 = cut5var.ReadBlock(dataindex, block);
                float * datacut6 = cut6var.ReadBlock(dataindex, block);
		for(int i =0; i < length; i++){
			if(!weights){

    //                            if(fabs(datacut[i]-1.02)>0.02){
  //                            //  if(datacut5[i]<0.75)
                                   // if((fabs(datacut1[i]-0.892)<0.08||fabs(datacut2[i]-0.892)<0.08)||(fabs(datacut3[i]-0.892)<0.08||fabs(datacut4[i]-0.892)<0.08))  continue;                              
	
  //                             if(!((fabs(datacut1[i]-0.892)<0.06||fabs(datacut4[i]-0.892)<0.06)||(fabs(datacut2[i]-0.892)<0.06||fabs(datacut3[i]-0.892)<0.06)))	
		        	datahist->Fill(data[i]);
//                                }
			} else {
//                                cout<<"weights[datacounter] "<<"datacounter   "<<datacounter<<"     "<<weights[datacounter]<<endl;
//                               if(fabs(datacut[i]-1.02)>0.02){
         //                           if((fabs(datacut1[i]-0.892)<0.08||fabs(datacut2[i]-0.892)<0.08)||(fabs(datacut3[i]-0.892)<0.08||fabs(datacut4[i]-0.892)<0.08))  continue; 
//                              if(!((fabs(datacut1[i]-0.892)<0.06||fabs(datacut4[i]-0.892)<0.06)||(fabs(datacut2[i]-0.892)<0.06||fabs(datacut3[i]-0.892)<0.06)))	
//                                if(fabs(datacut5[i])>0.8||fabs(datacut6[i])>0.8)
                               	datahist->Fill(data[i],weights[datacounter]);
  //                             if((fabs(datacut1[i]-0.892)<0.08&&fabs(datacut4[i]-0.892)<0.08)||(fabs(datacut2[i]-0.892)<0.08&&fabs(datacut3[i]-0.892)<0.08))
//                               }
			       datacounter++;
			}
		}
		...
		Reset(dataindex,block);
                cutvar.Reset(dataindex,block);
                cut1var.Reset(dataindex,block);
                cut2var.Reset(dataindex,block);
                cut3var.Reset(dataindex,block);
                cut4var.Reset(dataindex,block);
                cut5var.Reset(dataindex,block);
                cut6var.Reset(dataindex,block);
	}
	...
	for(int block =0; block < maxblock; block++){
		...
		float * mc = ReadBlock(mcindex, block);
                float * mccut = cutvar.ReadBlock(mcindex, block);
                float * mccut1 = cut1var.ReadBlock(mcindex, block);
                float * mccut2 = cut2var.ReadBlock(mcindex, block);
                float * mccut3 = cut3var.ReadBlock(mcindex, block);
                float * mccut4 = cut4var.ReadBlock(mcindex, block);
                float * mccut5 = cut5var.ReadBlock(mcindex, block);
                float * mccut6 = cut6var.ReadBlock(mcindex, block);
		...
		for(int i =0; i < mclength; i++){
			if(dcs[0][i+block*GPUStreamTensor<float>::GPUDataDependentObject::Blocksize] > 0){
                                //if(fabs(mccut[i]-1.02)>0.02&&(fabs(mccut5[i])>0.8||fabs(mccut6[i])>0.8)){
 //                               if(fabs(mccut[i]-1.02)>0.02){
       //                        if(mccut5[i]<0.75)
//                                        if((fabs(mccut1[i]-0.892)<0.08||fabs(mccut4[i]-0.892)<0.08)||(fabs(mccut2[i]-0.892)<0.08||fabs(mccut3[i]-0.892)<0.08))  continue;
       //                                if(!((fabs(mccut1[i]-0.892)<0.06||fabs(mccut4[i]-0.892)<0.06)||(fabs(mccut2[i]-0.892)<0.06||fabs(mccut3[i]-0.892)<0.06))) 
...
//                                if(mccut5[i]<0.75)
  ...
  //                                               }
		...
		delete [] mc;
		Reset(mcindex, block);
                cutvar.Reset(mcindex, block);
                cut1var.Reset(mcindex, block);
                cut2var.Reset(mcindex, block);
                cut3var.Reset(mcindex, block);
                cut4var.Reset(mcindex, block);
                cut5var.Reset(mcindex, block);
                cut6var.Reset(mcindex, block);
