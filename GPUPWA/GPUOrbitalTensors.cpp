...
GPUOrbitalTensors::GPUOrbitalTensors(GPUDataDependentObjectList *_list, GPUStreamVector &_pa, GPUStreamVector &_pb, GPUStreamVector &_pc, float _r) : GPUDataDependentObject(_list), mstreampa(&_pa), mconstpa(0), mpb(_pb), mpc(_pc), mRadius(_r)
{
	mmetric = new GPUMetricTensor();
	mpa = mstreampa;
	MakeTensors();
}
GPUOrbitalTensors::GPUOrbitalTensors(GPUDataDependentObjectList *_list, GPUConstVector &_pa, GPUStreamVector &_pb, GPUStreamVector &_pc, float _r) : GPUDataDependentObject(_list), mstreampa(0), mconstpa(&_pa), mpb(_pb), mpc(_pc), mRadius(_r)
{
	mmetric = new GPUMetricTensor();
	mpa = mconstpa;
	MakeTensors();
...
GPUOrbitalTensors::GPUOrbitalTensors(GPUDataDependentObjectList *_list, float4 &_pa, GPUStreamVector &_pb, GPUStreamVector &_pc, float _r) : GPUDataDependentObject(_list), mstreampa(0), mconstpa(new GPUConstVector(_pa)), mpb(_pb), mpc(_pc), mRadius(_r)
{
	mmetric = new GPUMetricTensor();
	mpa = mconstpa;
	MakeTensors();
...
void GPUOrbitalTensors::MakeTensors()
{
	...
	mB3 = &sqrt(227.0f / (Q6abc + (6.0f * Q_02) * Q4abc + (45.0f * Q_04) * Q2abc + 225.0f * Q_06));
	...
	if (mstreampa)
	{
		...
		//	  mSpin2OrbitalTensor = & (((*mr_tilde)%(*mr_tilde)) - 1.0f/3.0f * (*mr_tilde2) * (*mguv_tilde_pa_stream));
		//mr_tilde2->Print();
		...
		// mSpin3OrbitalTensor = &spin3orbitaltensor(*mr_tilde,  *mstreampa);
	...
	else
	{
		...
		//mSpin2OrbitalTensor = & (((*mr_tilde)%(*mr_tilde)) - 1.0f/3.0f * (*mr_tilde2) * (*mguv_tilde_pa_const));
		///
		...
		//mSpin3OrbitalTensor = &spin3orbitaltensor(*mr_tilde,  *mconstpa);
