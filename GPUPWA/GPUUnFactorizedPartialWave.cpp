...
GPUDataStream<float2> * GPUUnFactorizedPartialWave<float4>::Contract(GPUPartialWave * wave2, unsigned int streamindex, unsigned int block){
	...
	GPUFactorizedPartialWaveRes<float4> * castreswave2 = dynamic_cast<GPUFactorizedPartialWaveRes<float4> *>(wave2);
	if(castreswave2){

                GPUPropagatorResolution & prop1a = castreswave2->GetPropagator();
                GPUDataStream<float2> * sum1 = prop1a.SumResolution(streamindex, block);

                kernelcontractmesons_v_s(mPropagator.GetList()->GetDeviceInterface(),
                                                  castreswave2->GetTensor()(streamindex,block),
                                                  mPropagator.realpart(streamindex,block),
                                                  mPropagator.imagpart(streamindex,block),
                                                                        sum1,
                                                                        aiaj);
	...
	if(ucastwave2){
        ...
        GPUUnFactorizedPartialWave2P<float4> * ucastwave2phi = dynamic_cast<GPUUnFactorizedPartialWave2P<float4> *>(wave2);
        if(ucastwave2phi){
                        GPUPropagatorResolution & prop2a = ucastwave2phi->GetPropagator2();
                        GPUDataStream<float2> * sum2 = prop2a.SumResolution(streamindex, block);
                                        kernelcontractmesons_v_vs(mPropagator.GetList()->GetDeviceInterface(),
                                                                mPropagator.realpart(streamindex,block),
                                                                mPropagator.imagpart(streamindex,block),
                                                                ucastwave2phi->GetPropagator().realpart(streamindex,block),
                                                                ucastwave2phi->GetPropagator().imagpart(streamindex,block),
                                                                sum2,
                                                                aiaj);
                             delete sum2;
