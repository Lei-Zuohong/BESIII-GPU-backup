...
#pragma OPENCL EXTENSION cl_nv_printf:enable //for N
...
float d(float m0, float q0){
  float d = 3.0f/math_pi*mass_Pion*mass_Pion/(q0*q0)*log((m0+2.0f*q0)/(2.0f*mass_Pion))
					 + m0/(2.0f*math_pi*q0)
...
float wid(float mass, float sa, float sb, float sc, float r, int l){
  ...
  float F = 0.0f;
  if(l == 0) F = 1.0f;
  if(l == 1) F = sqrt((1.0f+z0)/(1.0f+z));
  if(l == 2) F = sqrt((9.0f+3.0f*z0+z0*z0)/(9.0f+3.0f*z+z*z));
  float t = sqrt(q/q0);
...
//__kernel void kernelRBW(__global float * mx2in, float mr, float wr, float m1_2, float m2_2, float r, int l, __global out float2 * output){
__kernel void kernelRBW(__global float * mx2in, float mr, float wr, __global float * m1_2in, __global float * m2_2in, float r, int l, __global out float2 * output){
	uint i = get_global_id(0);
	float mx2 = mx2in[i];
	float m1_2 = m1_2in[i];
	float m2_2 = m2_2in[i];
  ...
  float q = fundecaymomentum(mx2, m1_2, m2_2);
  float q0 = fundecaymomentum(mr2, m1_2, m2_2);
  float denom_real = mr2-mx2;
  float denom_imag = mr*wr*wid(mr,mx2,m1_2,m2_2,r,l);//real-i*imag;

	float denom = denom_real*denom_real+denom_imag*denom_imag;
	output[i].x = denom_real/denom;
	output[i].y = denom_imag/denom;
}

__kernel void kernelKPiSLASS(__global float * mx2in, __global float * m1_2in, __global float * m2_2in, __global out float2 * output){
  uint i = get_global_id(0);
  float sa = mx2in[i];
  float sb = m1_2in[i];
  float sc = m2_2in[i];
  float m1430 = 1.463f;
  float w1430 = 0.233f;
  float sa0 = m1430*m1430;
  float q0 = (sa0+sb-sc)*(sa0+sb-sc)/(4.0f*sa0)-sb;
  if(q0<0) q0 = 1e-16f;
  float qs = (sa+sb-sc)*(sa+sb-sc)/(4.0f*sa)-sb;
  float q = sqrt(qs);
  float width = w1430*q*m1430/sqrt(sa*q0);
  float temp_R = atan(m1430*width/(sa0-sa));
  if(temp_R<0) temp_R += math_pi;
  float deltaR = -5.31f + temp_R;
  float temp_F = atan(2.0f*1.07f*q/(2.0f+(-1.8f)*1.07f*qs));
  if(temp_F<0) temp_F += math_pi;
  float deltaF = 2.33f + temp_F;
  output[i].x = 0.8f*sin(deltaF)*cos(deltaF) + sin(deltaR)*cos(deltaR + 2.0f*deltaF);
  output[i].y = 0.8f*sin(deltaF)*sin(deltaF) + sin(deltaR)*sin(deltaR + 2.0f*deltaF);
}
