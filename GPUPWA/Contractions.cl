 ...
 __kernel void kernelcontractmesons_v_v(__global float4 * propagator1real, __global float4 * propagator1imag, __global float4 * propagator2real, __global float4 * propagator2imag, __global out float2 * result)
{
 ...
 __kernel void kernelcontractmesons_v_vs(__global float4 * propagator1real, __global float4 * propagator1imag, __global float4 * propagator2real, __global float4 * propagator2imag, __global float2 * contract, __global out float2 * result)
{
	 uint i = get_global_id(0);
        
	float4 r = (propagator1real[i]*propagator2real[i]+propagator1imag[i]*propagator2imag[i]);
	float4 im = (-propagator1real[i]*propagator2imag[i]+propagator1imag[i]*propagator2real[i]);
/*        float2 rrx = 0.5f*(r.x + r.y);	*/
/*        float4 rry = 0.5f*(im.x + im.y);	*/

        result[i].x = contract[i].x*0.5f*(r.x + r.y) - contract[i].y*0.5f*(im.x + im.y);
	result[i].y = contract[i].x*0.5f*(im.x + im.y) + contract[i].y*0.5f*(r.x + r.y);
}
