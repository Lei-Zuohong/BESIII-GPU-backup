...
//////////////////////////zhangyt
GPUStreamTensor2 & operator|  (GPUConstTensor4 & _lhs, GPUStreamTensor2 & _rhs){
        float4444 temp = _lhs(); 
	return * new GPUTwoStreamOp<float44, GPUStreamTensor2, float4444, GPUStreamTensor2 &,'|',false, true>(temp,_rhs);
}
////////////////////////
...
template<> GPUDataStream<float44>* GPUTwoStreamOp<float44, GPUStreamTensor2,float4444, GPUStreamTensor2 &,'|',false,true>::operator()(int index, int block){
	this->assertindex(index); if(this->mstream[index][block]) return this->mstream[index][block];
//	GPUDataStream<float44> * lstream = this->lhs(index,block);
	GPUDataStream<float44> * rstream = this->rhs(index,block);
	// Contracting Tensor4 stream with const Tensor2
	...
	GPUDataStream<float> * temp[16];
	for(int i=0; i<16; i++){
	  temp[i] = new GPUDataStream<float>(mList->GetDeviceInterface(), this->rhs.GetLength(index,block));
	  kerneltensorcontract_f4444_2(mList->GetDeviceInterface(),lhs(i), (rstream), (temp[i]));
	}
	kerneltensormake_2(mList->GetDeviceInterface(),(temp[0]), (temp[1]), (temp[2]), (temp[3]),
			   (temp[4]), (temp[5]), (temp[6]), (temp[7]),
			   (temp[8]), (temp[9]), (temp[10]), (temp[11]),
